...
def deleteproblem(request,id):
    pcost = problemcost.objects.get(id=id)
    pcost.delete()
    return redirect('../')
...
def create_problem(request):
    ...
    from .models import problemcost
    problemcost.objects.create(ProblemDescription=description, Cost=cost)
...
def customer_dashboard_view(request):
    ...
    print(bill)
    prcost = problemcost.objects.all().order_by("Cost")
    dict={
